...
class TopRatedMoviesPage extends Component {
  ...
  getMoviesData = async () => {
    const response = await fetch(
      'https://api.themoviedb.org/3/movie/top_rated?api_key=ac116e4cc4bc7630fbd0f390a00f6007&language=en-US&page=1',
    )
  ...
  render() {
    ...
    <h1>Top Rated</h1>
      <ul className="posters-container">
        {movieslist.map(item => {
          const image_url = 'https://image.tmdb.org/t/p/w500' + item.posterPath
          return (
            <EachPoster
              key={item.id}
              posterDetails={item}
